...
export const ProjectItem: React.FC<ProjectItemProps> = ({ project, index }) => {
  ...
  //   ? "ml-[-5vw] md:pl-[5vw] rounded-r-xl border-l-0" // Stretches left off-screen
  ...
  return (
    <div 
      className={`flex flex-col md:flex-row bg-white border border-slate-200 overflow-hidden shadow-sm transition-all duration-300 w-full hover:shadow-md hover:border-blue-200/80 hover:bg-slate-50/50 group`}
    >
      ...
      <div className={`flex-1 p-6 md:p-8 flex flex-col justify-between ${textOrderStyles}`}>
        <div>
          ...
          {/* Title & Description */}
          <h3 className="text-2xl font-extrabold tracking-tight text-slate-950 mb-2 group-hover:text-blue-600 transition-colors">
            {title}
          </h3>
          <p className="text-slate-950 text-base leading-relaxed mb-6">
            {description}
          </p>
        </div>
